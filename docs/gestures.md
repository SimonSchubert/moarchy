...
| **app drawer** | The searchable app grid (`moarchy.app-drawer`). The strip's sheet as shipped. |
| **workspace overview** | The vertical list of workspace cards (`moarchy.workspace-overview`). The right edge's sheet as shipped. It is the one surface that can put two apps on one workspace, and the one place a window is closed by hand. |
| **control center** | The pull-down from the top edge (`moarchy.control-center`). |
| **the strip's sheet** | What an up-swipe from the strip raises. Named by a setting (Q1), so §A says *the app drawer* where it means *this*, and its checks run against the pairing that ships. |
...
| **travel** | Drag distance as a fraction of the sheet being dragged, along the axis it arrives on — the app drawer's own height, ~694 logical px. One pixel of finger is one pixel of sheet, on every surface that drags it (D2a). |
...
both directions — a short fast flick up opens the app drawer, a fast flick down
...
leaves `omarchy-shell app-drawer state` == `open` and a 2000ms drag leaves it
...
→ a drag of *n* logical px from the strip leaves `app-drawer dragTrace` ending
...
→ the same 35% flick, eight times over, leaves `omarchy-shell app-drawer state` ==
...
already-open app drawer it is one short pull more (A6), which is the path that
actually gets used. Released there, focus lands on a home screen and the app drawer
...
→ focused workspace `representation` is empty; `app-drawer state` == `closed`
...
over it (A8).
→ `app-drawer state` goes `closed` → `open` across a strip up-gesture, from an app
and from a home screen alike; with the control center down it stays `closed` and
`control-center state` goes to `closed`

**A6** With the app drawer already open, dragging up from the strip again carries on
...
→ from an open app drawer, a 20% drag leaves `representation` empty and
`app-drawer state` == `closed`

**A7** A short up-swipe with the app drawer already open leaves it open. The strip
does not toggle it: up means "forward" — to the app drawer, then to home — and
never "back". What closes the app drawer is a drag *down* on the sheet itself (H1)
or the back gesture (G3). This is what A6's "measured from where the drag
began" buys: against a fixed threshold an open app drawer is already past the stop
before the finger moves, so every touch on the strip would go home.
→ from an open app drawer, a 5% drag leaves `app-drawer state` == `open`

**A8** With the control center down, an up-swipe from the strip puts the control center away and
...
app drawer here is what would make a second drag clear the workspace overview instead of
carrying it on into the home band, the moment somebody put the workspace overview on
this edge.
→ `omarchy-shell control-center state` == `closed`; nothing else opened, and a app drawer
that was open under the control center is still open

**A10** An up-swipe from the strip with the control center over an open app drawer puts the
control center away and leaves the app drawer where it is. One gesture clears one covering
sheet, so what is on screen afterwards is the app drawer the control center was over.
→ with both open, a strip up-flick leaves `omarchy-shell control-center state` ==
`closed` and `omarchy-shell app-drawer state` == `open`
...
oversight. It is the deliberate one; collapsing the two takes the app drawer with
the control center.
...
it. The control center, the app drawer and the theme picker are put away on the way, the way
an up-swipe puts the control center away (A8).
...
coding agent**: the agent the app drawer's one tile names, in a terminal, on its
...
→ a 1200ms press that travels 500px up leaves the app drawer open, `hold=idle`,
and no agent window
...
window for anyone with a keyboard, and the workspace overview's bin closes one you can
see (P12).
...
window that opens underneath the app drawer is a gesture that appears to have done
...
→ with the app drawer open, `omarchy-shell gestures hold` leaves `app-drawer state` ==
...
→ `omarchy-shell app-drawer state` == `open`, `app-drawer dragTrace` ≥ 8 samples

**D2** Released below halfway the app drawer animates back down; above halfway it
...
→ a drag of *n* logical px leaves `app-drawer dragTrace` ending within a few
percent of `n / 694`; measured 300px→42%, 435px→63%, 635px→91%

**D2b** The number that drag divides by is right **while the app drawer is
...
`app-drawer geometry` answers `h=100` with the app drawer down and `h=694` with it up,
...
The app drawer remembers its height the first time it is up and falls back to the
...
→ `omarchy-shell app-drawer geometry` reports `travel=720` before the app drawer has
...
workspace and the top edge still opens the control center.
...
was on afterwards, with the workspace overview still drawing a card that holds it.
→ `workspace-overview windows` count unchanged across a home gesture
...
wallpaper. So the app drawer has to leave from that travelled state and not from
...
→ `omarchy-shell app-drawer retireTrace` reports `progress:homeHint` per frame
...
not free: the app drawer takes exclusive keyboard focus for its search field, and
...
because the app drawer's pre-launch hop (windows.md L10) asks the same question and
...
→ `omarchy-shell gestures status` with the app drawer up over an app reports
...
2. each open overlay — app drawer, control center or theme picker — topmost first, one
   per swipe
3. the focused app
...
→ `omarchy-shell workspace-overview windows` is one line shorter
...
**G9** The left-edge swipe is back and the right-edge swipe is the workspace overview (P1) — one 16px
...
`panelHeight` I5b pins the app drawer's reflow to), so unlike G8 it does **not** go
...
allocated: the control center's grab strip is Overlay too and covers the same 16x26, and
which of two Overlay surfaces got a touch there was decided by map order rather
than by anything this spec says. Above 73 the control center has it outright (A8).
→ `omarchy-shell gestures geometry` reports `h` == `screen - inset - topInset`
and `topInset` == 73
...
layer order — the control center is Overlay, the app drawer and the theme picker are Top —
so the sheet that goes is always the one being looked at.

`overlayIds` leads with the control center for that reason and not by accident, so
reordering it changes which sheet a back swipe reaches first.
→ with the control center over an open app drawer, one `omarchy-shell gestures back` leaves
`control-center state` == `closed` and `app-drawer state` == `open`; a second leaves both
...
back swipe (G2). Opening the app drawer, closing it, and switching apps leave it where it was.
...
`Visible` stays false across ~3s of sampling; going home, opening the app drawer
without touching its field, and switching apps leave it wherever it was

**G14a** Tapping the app drawer's search field raises the keyboard. Closing the app drawer does
not lower it.
...
is what raised the keyboard on every app drawer open, and hiding on close is what put it
...
opened with `app-drawer open`. The keyboard that tap raised stays up when the app drawer goes
down: back (G2) is the one way down.
→ with the keyboard down, `omarchy-shell app-drawer open` then a tap at
`app-drawer searchTarget`'s field centre leaves the focused workspace's `rect.height`
lower by the keyboard's reservation, and `searchTarget` reports `focused=true`;
closing the app drawer leaves the rect lowered. Open and close without touching the
...
**H1** Dragging **down** anywhere on the app drawer closes it, following the finger.
→ `omarchy-shell app-drawer dragTrace` leaves ≥ 8 samples; `app-drawer state` == `closed`

**H2** Dragging **up** anywhere on the control center closes it, following the finger.
...
its content and stops at 90% of the usable height (`control-center.md` S21, S22), so the
band is ~70px with the control center full and several hundred with it near empty.

`state` alone cannot check this. A control center that jumps shut reaches `closed`
...
→ `omarchy-shell control-center dragTrace` leaves ≥ 8 samples; `control-center state` == `closed`
...
→ launching from the app drawer still works after H1 is implemented

**H5** Where the sheet's own content scrolls — the app drawer's grid when it has
more apps than fit, the control center's notification list — that content gets the drag
...
control center is closed and reopened
...
**H7b** Neither a swipe nor a vertical drag over the list closes the control center.
...
control center from depending on a gesture nobody told you about.
...
**I1** With the app drawer, the theme picker or the keyboard up, the surface
...
window is drawn in; and it is below every sheet, so the app drawer, the control center and
...
the app drawer and the theme picker. The `moarchy-home` surface takes the same
...
→ `omarchy-shell {app drawer,themes} geometry` each report `h` equal to
...
**I5** The app drawer still reflows above the on-screen keyboard -- the reason its
exclusive zone is zero in the first place. Raising the keyboard shortens the
app drawer's surface, and the grid ends the same distance above the keyboard as it
did before the surface grew.
→ `app-drawer geometry` `h` is smaller with the keyboard up than with it down, and
`gap` is identical between the two

**I5a** The app drawer's bottom inset is dropped while the keyboard is up, and this
...
is on Top like the app drawer and mapped earlier, so the app drawer wins the overlap and
paints over it — the whole `qwertyuiop` row reduced to a sliver under the
app drawer's app labels.
...
→ `app-drawer geometry` reports `margin=0` with the keyboard up and
...
→ with the keyboard up, `app-drawer geometry` `h` is `screen - bar - panelHeight`;
at 176 rather than 200 the app drawer settles over the top key row
...
→ with an app focused and the keyboard up, open and close each of the app drawer,
...
→ with the app drawer open, the keyboard forced up on `sm.puri.OSK0` and the search
field never tapped, `app-drawer geometry` reports `margin=0` while `searchTarget`
...
never swallowed. The control center is the surface that needs a mask for the pill, and
only because it is on Overlay itself.
→ A7 with the app drawer; `omarchy-shell themes state` == `closed` after an
...
`4A3E53` on `150D20`, and over the app drawer's sheet to `45485B` on `1A1B26`. Both
...
the workspace overview draws for one a real window rather than a stand-in.
...
node that is the only window on its workspace, and `workspace-overview windows` names
`moarchy.settings`
...
it leaves any app running. Its tile is on its card in the workspace overview and tapping
...
`workspace-overview windows` still names `moarchy.settings`, and `settings state` == `open`

**K5** A shell app names and draws itself: the glyph the control center opens it by, the
...
that knows, and `workspace-overview windows` prints the pair.
...
`workspace-overview windows` prints that plugin's id and the page it is on

**K6** Two things close a shell app, and both drop its tile and reset the page
stack: dropping the tile in the workspace overview's bin (P12), and the back gesture with
...
→ after either, `workspace-overview windows` has no `moarchy.settings` line and
...
nothing else. The control center and the app drawer stay transient sheets with no tile of
...
network means retyping a passphrase and coming back; Bluetooth (`control-center.md` S6d)
...
window per screen, and the ways in are many — the control center's gear, a Settings row,
the app drawer, an IPC verb. Naming a page still navigates (`settings.md` A7); it is
...
`workspace-overview windows` with one `moarchy.settings` line
...
**Removal is the app drawer's, not a terminal's.** Upstream's
...
it is stated in the one surface that costs a keyboard to answer. So the app drawer
...
`omarchy-shell app-drawer cellTarget 0`, leaves `omarchy-shell app-drawer detail`
...
→ after the hold, `omarchy-shell workspace-overview windows` gained no line
...
→ a 1200ms drag down from a cell leaves `app-drawer state` == `closed`,
`app-drawer detail` empty, and `app-drawer dragTrace` ≥ 8 samples
...
**L5** The detail sheet is a **card over the app drawer**, not a surface and not a
page. The app drawer keeps its keyboard focus, its scroll position and its progress;
...
of the app drawer — because the plan is a step you took and back is the step you
take to undo one.
→ from an armed plan, three `omarchy-shell gestures back` calls give stage
`info`, then no card with `app-drawer state` == `open`, then `closed`
...
→ `omarchy-shell app-drawer detail` prints `id`, `kind`, and for a package
...
with pacman's own line, and `omarchy-shell app-drawer canRemove` is `no`
...
**L10** The grid drops the app when it is gone, without reopening the app drawer.
...
→ a hold on `moarchy.device` reports `info.protected=1`, `app-drawer uninstall` ==
`protected` and `app-drawer canRemove` == `no`
...
Measured against all 34 entries the app drawer lists, three undeclared dependencies
...
So the app drawer would have offered to remove the terminal every TUI opens in, and
...
## N. Opening the app drawer

The sheet is dragged open by a finger at 60Hz on a phone GPU, and everything the
open path does lands on the frames the sheet is arriving on. The control center is the
...
**N1** Opening the app drawer starts no filesystem scan. The icon index refreshes
...
running has its icon without the app drawer asking. One scan per shell start
...
→ `grep -c 'appLibrary.refreshIcons()' moarchy.app-drawer/AppDrawer.qml` is 1, and
that call sits behind a flag `open()` sets; on the device, a second
`omarchy-shell app-drawer open` spawns no `find` under the shell —
`pgrep -af 'find .*icons' -P $(pgrep -x quickshell)` during the open is empty

**N3** The app drawer's surface is never unmapped. Shut, it is a one-pixel band along
...
away for a release.
→ tap the search field, close the app drawer, open it again:
`omarchy-shell app-drawer searchTarget` reports `focused=false`

---

## P. Right-edge swipe — the workspace overview
...
**P1** Swiping in from the **right edge** raises the edge's sheet — the workspace overview
...
→ `omarchy-shell gestures geometry` reports `workspaceOverviewW` == 16 scaled and
`workspaceOverviewSurfaceW` == `workspaceOverviewW`
...
either way decides whatever the travel. That is the app drawer's release rule read on
the other axis, through the same `sheetCommit`.
→ `omarchy-shell workspace-overview dragTrace` leaves ≥ 8 samples across a slow drag in
...
→ `omarchy-shell workspace-overview grid` prints one `ws=` line per numbered workspace with
...
both close the workspace overview. The focused workspace's card and the card a drop would
land on are marked the same way, in the accent — this shell's existing word for
"this is where you would end up" (A4, C2).
→ tapping a tile leaves `omarchy-shell workspace-overview state` == `closed` and the
...
app drawer asks whether an app is already running before it hops a workspace to
launch it (`windows.md` L10). One index, because two that answer the same
question disagree the day one of them goes stale.
→ `grep -c 'Apps\.' moarchy.app-drawer/AppDrawer.qml moarchy.workspace-overview/WorkspaceOverview.qml` is
...
card at the end.
→ `omarchy-shell workspace-overview lift <ws> <index>` then `workspace-overview dropOn <ws>` moves
that window, and `workspace-overview move <con_id> <ws>` is the same call without the
gesture: `workspace-overview grid` shows it under the second `ws=` and no longer under the
first. With a finger, aimed with `workspace-overview tileTarget <ws> <index>`:
...
X1 Y1 900` on a tile is a *tap* (P4): `workspace-overview state` == `closed` with the
...
a keyboard user's `$mod+Shift+2` lands the same way as the drag. The workspace overview
...
→ `omarchy-shell workspace-overview grid`'s `free=` equals
...
→ `omarchy-shell gestures geometry` reports `workspaceOverviewH` == `h`
...
while it is up.
→ `grep -c 'surfaceUp' moarchy.workspace-overview/WorkspaceOverview.qml` guards both the refresh and
the two `Connections`

**P10** The workspace overview is reachable without a finger.
→ `omarchy-shell workspace-overview open|close|toggle|state|progress|grid|move|lift|dropOn`,
and `omarchy-shell gestures workspace overview` for the summon the edge performs

**P11** The workspace overview is a sheet, so the rules about sheets are the rules about it:
it puts away the app drawer and the theme picker beside it and the control center above it
...
→ with the workspace overview up, one `omarchy-shell gestures back` leaves
`workspace-overview state` == `closed` with the open-window count unchanged
...
→ `omarchy-shell workspace-overview binTarget` reports `drawn=false` and the rect the
bin will occupy while nothing is lifted, and `drawn=true` after
`workspace-overview lift <ws> <index>`; `workspace-overview aim <x> <y>` into that rect leaves
`workspace-overview lifted` ending `over bin`, and a point over a card leaves it ending
`over <n>`
...
quit is still running and has its tile again the next time the workspace overview comes
up.
...
→ `workspace-overview lift <ws> <index>` then `workspace-overview trash` leaves
`workspace-overview tileTarget <ws> <index>` == `no tile` before the app has answered and
`workspace-overview windows` one line shorter once it has, with `workspace-overview grid` still
printing a `ws=` line for the workspace it was on. With a finger, a
`sudo moarchy-touch drag` from `workspace-overview tileTarget` to `workspace-overview binTarget`'s
...
written in the code: the strip raised `moarchy.app-drawer`, the right-edge swipe raised
`moarchy.workspace-overview`, the hold started the coding agent, and the power button had
...
The values are words -- `none`, `app drawer`, `workspace overview`, `control center` -- and not plugin
ids, so `ui.toml`, `bin/moarchy-ui` and the Settings rows never spell one and
`Sheet.js` stays the only place the ids live. Shipped they are the app drawer and
the workspace overview, so a phone nobody has touched behaves exactly as §A and §P
describe.
→ with no `ui.toml`, `moarchy-ui get gesture-bottom` == `app drawer` and
`moarchy-ui get gesture-right` == `workspace overview`; `omarchy-shell gestures status`
reports `bottom=moarchy.app-drawer right=moarchy.workspace-overview`

**Q2** A sheet arrives from the edge that raised it, and its own close drag runs
back along the same axis. The workspace overview on the strip rises from the bottom rather
...
→ with `gesture_right` == `app drawer`, a drag in from the right edge leaves
`omarchy-shell app-drawer dragTrace` with ≥ 8 samples rising monotonically, and
`app-drawer geometry` reports a `travel` of the screen's width where the same sheet
...
The control center is what this is for: it is as tall as its content (`control-center.md` S21) and
...
→ with `gesture_right` == `control center`, `omarchy-shell control-center geometry` reports the
...
→ with the app drawer open, `moarchy-ui gesture-bottom workspace overview` leaves
`omarchy-shell app-drawer geometry` unchanged; the next strip drag raises the
workspace overview
...
sheet's own -- the control center's grab band means *down from the status bar* whatever
the strip is pointed at; and a summon that carries no direction at all (an IPC
verb, the control center's gear, a tap trigger) uses the sheet's natural edge. All
...
the control center, `entryEdge` was left at `bottom` by the last strip press, so a pull
*down* from the status bar slid the sheet up off the floor and left the top
two thirds of the screen empty.
→ with `gesture_bottom` == `control center`, a pull down from the status bar leaves the
...
error that degrades gracefully: the control center froze its height on `beginDrag()`,
...
→ with `gesture_bottom` == `control center`, an up-swipe from the strip at 400ms, 900ms
and 1500ms each leave `omarchy-shell control-center state` == `open`, and
`control-center dragTrace` rises to ≈ 100
...
latches.
→ with the workspace overview disabled and `gesture_right` == `workspace overview`, a drag in from
...
second right-edge drag on an open workspace overview has nowhere further to go. The strip
stays the exception it already is: a second drag there runs on into the home
band (A6).
→ with both keys == `app drawer` and the app drawer open, a right-edge drag leaves
`app-drawer dragTrace` empty and `app-drawer state` == `open`; a strip drag from there
leaves `representation` empty

**Q7** The control center keeps its own way in. Its grab band across the status bar
(`control-center.md` H2) raises it whatever edge it is configured on, and raises it
*from the top* (Q3a) -- so a control center set on an edge has two ways up, each
arriving from the edge the finger started on, and a setting meant to add one
takes none away.
→ with `gesture_bottom` == `control center`, a pull-down on the status bar leaves
`omarchy-shell control-center state` == `open`
...
→ `moarchy-ui gesture-right control center`, then with no restart a right-edge drag
leaves `omarchy-shell control-center state` == `open`
...
agent, or **any app the app drawer lists**. The value is a word for the first
...
per line of `omarchy-shell app-drawer entryRows`; with `gesture_hold` == `workspace overview`,
`moarchy-trigger fire hold` leaves `omarchy-shell workspace-overview state` == `open`

**Q10a** An app opens the way the app drawer opens it. `omarchy-shell app-drawer
...
→ firing an app trigger leaves `workspace-overview windows` one line longer, with the new
window alone on a free workspace

**Q10b** The names on the list are the app drawer's names. `app-drawer entryRows`
answers id and name together from the same walk `app-drawer entries` makes, so a
trigger can neither offer an app the grid hides nor call one something the grid
does not.
→ `omarchy-shell app-drawer entryRows | cut -f1` equals `omarchy-shell app-drawer
entries`
