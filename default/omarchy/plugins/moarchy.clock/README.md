...
|---|---|---|
| `moarchy.clock` | open | **closed** |
| `moarchy.calculator` | open | open |
| `moarchy.keep` | open | open |
| `moarchy.coins` | open | open |
...
docker run --rm --platform linux/arm64 -v "$PWD:/src" -w /src moarchy-qml \
  scripts/qml-check.sh moarchy.clock
```
...
```sh
plugins/moarchy.clock/run-local.sh
MOARCHY_CLOCK_PAGE=stopwatch plugins/moarchy.clock/run-local.sh
```
...
The pictures are `scripts/qml-shot.sh moarchy.clock`, and `shots.sh` says
