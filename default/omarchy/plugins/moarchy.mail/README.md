...
```sh
plugins/moarchy.mail/run-local.sh
```
...
```sh
docker run --rm --platform linux/arm64 -v "$PWD:/src" -w /src moarchy-qml scripts/qml-check.sh moarchy.mail
docker run --rm --platform linux/arm64 -v "$PWD:/src" -w /src moarchy-qml plugins/moarchy.mail/tests/e2e.sh
```
